...
export default class AppContextProvider extends React.Component{
    ...
    addComment = (postId,comment) => {
        const tmp = [...this.state.posts];
        const index = tmp.findIndex(p => p.postId === postId);
        const post = {...tmp[index],comments:[...tmp[index].comments,comment]};
        tmp[index] = post;
        this.setState({...this.state,posts:tmp});
    }
